...
For Nazih: at exam registration, per sitting, as a condition the student sees in full, from the candidate, who owns the line and declares their own exam address. Checks run inside the exam window only. Nothing is recorded between checks and no location is ever retrieved, only verified.
