# Nazih - Prototype Phase submission
...
**YOU.** Pick your Idea Phase submission for Nazih from the dropdown.
...
1. The dashboard after Run all seven exam events, showing the batch table and the cost comparison.
...
```
Nazih Pitch Deck.pptx
```

It sits in the parent folder of this bundle. Update the closing slides with
the prototype numbers before uploading: 7 scenarios, all passing,
7 CAMARA APIs, 69.2% cheaper than calling every check.
...
```
https://github.com/mert-gng-99/nazih-nac-agent
```
...
| --- | --- | --- |
| Parent Submission | Pick Nazih from the dropdown | seconds |
...
| --- | --- |
| At least one CAMARA API on Nokia NaC | 7 API families: `number-verification`, `location-verification`, `device-swap`, `sim-swap`, `device-status`, `congestion-insights`, `quality-on-demand` |
...
| Solves a real problem | Open Innovation. Stated limits are in README.md, so this is not pitched as doing more than it does. |
| Theme alignment | Theme 7 |
| Multiple CAMARA APIs (good-to-have) | 7 families orchestrated per case |
| Intelligent orchestration (good-to-have) | 2 to 3 calls depending on the case; 69.2% cheaper than calling everything |
| Production-minded (good-to-have) | Consent in the transport path, evidence ledger, Docker and Render config, 7 scenario tests plus transport, agent and API suites |
