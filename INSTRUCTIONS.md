...
1. A sitting starts normally, the first scenario on the left. Watch the agent pick
...
2. Run all seven exam events runs all 7 scenarios back to back and shows the
...
and all 7 scenarios reaching the outcome they claim.
...
```bash
docker build -t nazih . && docker run -p 8000:8000 nazih
```
